...
class Feature():
    ...
    async def websocket_up(self):
        """
        For subclasses: this is called after a client has connected, but just
        before we enter the event loop for that client. So if a feature needs
        to do something pre-emptively, this is their chance. This base class
        implementation is just a default.
        """
        pass
    ...
    async def handle(self, req):
        ...
        try:
            ...
            await self._notify(nfn_type='error', endpoint='global',
        ...
        if result is not None:
            # Reflect back the mandatory "endpoint" and optional "id" from the
            # client, plus if the nfn_type is missing then insert the req_type
            # (as is typical for eg RPCs)
            result['endpoint'] = req['endpoint']
            if 'id' in req:
                result['id'] = req['id']
            if 'target' in req:
